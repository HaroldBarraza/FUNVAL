...
const addTask = async (title: string) => {
  try {
    ...
    console.error(`se necesita que ingrese un titulo valido`);
  }
};
const saveDB = (task: Task): Promise<void> => {
  return new Promise((resolve) => {
    console.log(
      `se esta agregando una nueva tareas espere porfavor ${task.title}`,
    );
    setTimeout(() => {
      resolve();
...
const removeTask = (id: number) => {
  try {
    if (!tituloTareas.find((numero) => numero.id === id)) {
      throw new Error("no existe ese numero de tarea");
    }
    const encontrar = tituloTareas.findIndex((numero) => numero.id === id);
    tituloTareas.splice(encontrar, 1);
  } catch (error) {
    console.error(error);
  }
};
...
const markCompleted = (numero_id: number) => {
  const tarea_select = tituloTareas.find(
    (id_tares) => id_tares.id == numero_id,
  );
  if (tarea_select) {
    tarea_select.completed = true;
    const { id, title } = tarea_select;
    console.log(
      `se actualizo correctamente la tarea ${id}, titulo ${title} estado completado`,
    );
  } else {
...
const filterCompleted = () => {
  if (tituloTareas.length != 0) {
    const completados = tituloTareas.filter(
      (estado) => estado.completed == true,
    );
    console.log(`las tareas completadas son :`);
    const filtrados = descontruccion_todos(completados);
    filtrados.forEach((verdaderos) => {
...
while (cumple) {
  let answer =
    await rl.question(`Ingrese el numero de la opcion que deseia realizar: \n
    1. Crear una tarea
    2. Eliminar Tarea por ID
  ...
  switch (answer) {
    case "1":
      let tarea = await rl.question(
        "Ingrese el la tarea que desea agregar: \n",
      );
      await addTask(tarea.trim());
      break;

    case "2":
      const eliminar = await rl.question(
        "introdusca el N° de tarea que desea eliminar",
      );
    ...
    case "4":
      listTask();
      let id_tarea = await rl.question(
        `ingrese el numero de la tarea que desea cambiar el estado`,
      );
